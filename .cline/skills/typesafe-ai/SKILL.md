...
license: MIT
description: >
  Build AI-powered software with TypeSafe on Venice.ai: small units of AI
  intelligence you can use like programming primitives. Jev, the System One
  decision model, turns natural language and application state into typed
  judgments and probabilities that code can combine. Use when a feature needs
  programmable common sense, when brainstorming what AI could make possible in
  an app, or when an LLM prompt-and-parse step could become a structured
  decision. Applications include routing, ranking, extraction, verification,
  and interactive experiences; these are starting points, not the limits.
  Hosted on Venice.ai — one API key reaches chat, image, video, audio, and
  decisions. Read the Venice Jev page and TypeSafe cookbooks for patterns.
---

# Build with TypeSafe on Venice.ai
...
flagship and first System One model, now available on **Venice.ai**. It understands
natural language and returns typed answers and probabilities rather than generating
text or reasoning explanations. Code owns the workflow; the model supplies
programmable common sense where ordinary code needs semantic understanding.

## Venice.ai API

Jev is accessed through the Venice.ai Decisions API. The same Venice API key that
reaches chat, image, video, and audio also reaches Jev — no separate account needed.

### Endpoints

| Purpose | Method | URL |
| --- | --- | --- |
| Send decisions | `POST` | `https://api.venice.ai/api/v1/decisions` |
| TypeSafe SDK compat | `POST` | `https://api.venice.ai/api/v1/systemone` |
| List decision models | `GET` | `https://api.venice.ai/api/v1/models?type=decision` |

The `/systemone` endpoint serves the same handler as `/decisions`. TypeSafe SDKs
need only a **base URL swap** to `https://api.venice.ai/api/v1` to work with Venice.

### Authentication

All requests use a Bearer token. Set the `VENICE_API_KEY` environment variable or
pass it directly in the `Authorization` header:

```
Authorization: Bearer $VENICE_API_KEY
```

**Never expose the API key in client-side code.** Keep credentials server-side.
...
Use `"model": "jev-latest"` to reach the current Jev model. List available decision
models with `GET /models?type=decision` and treat the Models API as authoritative
since model IDs, pricing, and limits can change.
...
| State + all questions (one request) | 64K tokens |
| Input format | Plain strings, or structured JSON objects and arrays |
| Evaluation | All questions run in parallel against the same state |

### Pricing

| Item | Rate |
| --- | --- |
| Input tokens | $0.042 / 1M tokens (currently **free** during promo) |
| Output (answers) | **Free** — answers are not billed |

New Venice accounts include a free daily allowance and 500 welcome credits.
No credit card required to start. Jev is in **beta**; availability, behavior, and
schemas may change without notice.

### Privacy

Venice keeps **no copy** of the state you send or the answers it returns, and
passes nothing on for training. No state retention. No prompt retention.
No training on your data.

## Read the live docs

**Read the live docs as part of the task.** This skill gives direction; the docs
carry current concepts, prompting guidance, API contracts, SDK usage, models,
limits, and worked examples.

- Start with the [Venice Jev page](https://venice.ai/lp/jev) for the latest
  endpoint, pricing, and model details.
- Use the [TypeSafe documentation index](https://docs.typesafe.ai/llms.txt) to
  discover concepts, primitives, patterns, and cookbooks. Use targeted reads
...
| --- | --- |
| Check Venice API status & limits | [Venice Jev page](https://venice.ai/lp/jev) |
