...
| `cow_walk_eat_sheet_v2.png` | 7 equal cells (248×247), **faces right only**. Frames 0–3 walk, 4–6 eat. Each cow is fully inside its cell with pad; Pixi slices inset 2px so frames never share pixels. Anchor feet `(0.5, 0.94)`. Left roam uses `scale.x = -1`. |
