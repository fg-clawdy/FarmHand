...
| --- | --- | --- | --- |
| Terrain (dirt, farmland, hay paths), hay bales, sacks, crates, fences, ladder, planks, barn/store wall+roof+chimney pieces, corn growth tiles | [Kenney Isometric Miniature Farm](https://kenney.nl/assets/isometric-miniature-farm) | CC0 | `/art/vendor/kenney/iso-miniature-farm/` (`*_N.png`, 128×256), packed into the runtime atlas + a large `pixi-tiledmap` iso ground (farm 40×40, garden 28×22) with the homestead in the diamond center so Kenney grass reaches the viewport edges |
| Grass ground | Kenney `dirt_N` with a green recolor of the **top face only** (brown side facets kept) | CC0 derivative | iso map ground layer (the visible lawn — not a flat Graphics wash). Hay/crate/sack stay on the detail layer or as sprites; `hay_N` is a prop, not a ground tile. |
