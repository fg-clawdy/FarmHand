...
import SeedPouchSheet from "../components/SeedPouchSheet";
import PointsReviewSheet from "../components/PointsReviewSheet";
...
function GardenPlay({
  ...
  function celebrateWaitingSeed() {
    setJobToast(true);
    window.setTimeout(() => setJobToast(false), 5600);
  }
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => setBadgeQueue((q) => q.slice(1)), 7200);
  ...
  const { hostRef, sceneRef, ready } = useGardenPixi((slot) => {
    ...
    if (action === "harvest") {
      void run(async () => {
        const shardsBefore = player.seedShards;
        ...
        setOverlay(null);
        window.setTimeout(() => setGain((cur) => (cur === data.reward ? null : cur)), 8500);
        return data.player;
      });
      return;
    }
    ...
    <div className="screen garden-hybrid">
      ...
      <div className="topbar">
        ...
        <div className="meters">
          <button
            type="button"
            className={`meter meter-btn ${gain && gain.points > 0 ? "bump" : ""}`}
            aria-label={`${player.points} stars. Open farm story review.`}
            onClick={() => setOverlay({ type: "points-review" })}
          >
      ...
      <div className="garden-tools" role="toolbar" aria-label="Garden tools">
        ...
        {GARDEN_TOOLS.map((id) => {
          ...
          return (
            ...
            {overlay?.type === "points-review" && (
        <PointsReviewSheet onClose={() => setOverlay(null)} />
      )}
      {overlay?.type === "seed-stats" && config && (
        <SeedPouchSheet
          player={player}
          config={config}
      ...
      {overlay?.type === "selfie" && (
        <SelfieCapture
          ...
          onSuccess={(next, reward, unlocks) => {
            ...
            if (reward) {
              setGain(reward);
              window.setTimeout(() => setGain((cur) => (cur === reward ? null : cur)), 7200);
            }
          }}
        />
      )}
